...
@Component({
  selector: 'app-form-tasas-admincli',
  templateUrl: './form-tasas-admincli.component.html',
  styleUrls: ['./form-tasas-admincli.component.css']
})
export class FormTasasAdmincliComponent {
  @Input() data: any
  ...
  constructor(
    ...
    private tasasServices: TasasService
  ) {
  ...
  ngOnInit() {
    ...
    this.formGroup = new FormGroup({
      IdSegmento: new FormControl<any | null>(null)
    });
  }
  ...
  saveRegistro(): void {
    this.submitted = true;
    this.data.IdSegmento=this.selectedDropdown.Id
    if (this.data.Detalle?.trim()) {
      if (this.data.Id) {
  ...
  agregarData(row: any): void {
    this.tasasServices.postData(row).subscribe(response => {
      this.onClick.emit()
  ...
  actualizarData(row: any): void {
    ...
    this.tasasServices.putData(row.Id, row).subscribe(response => {
      this.onClick.emit()
